...
-   $H_{products}$ \< $H_{reactants}$, therefore ΔH is negative and heat
    energy released by the system (exothermic).

-   $H_{products}$ \> $H_{reactants}$, therefore ΔH is positive and heat
...
{{< figure src="https://cdn.statically.io/gh/psharma04/image-repo/main/uploads/hess-law.min.svg" height=100% width=100% theme="light" >}}
...
Josiah Willard Gibbs redefined the quantity −TΔ$S_{universe}$ as free
energy or Gibbs Free Energy, 𝚫G.
...
-   If 𝚫G \< 𝟎 (Δ$H_{system}$ \< TΔ$S_{system}$), a reaction is
    spontaneous
-   If 𝚫G \> 𝟎 (Δ$H_{system}$ \> TΔ$S_{system}$), a reaction is
    non-spontaneous
-   If 𝚫G = 𝟎, a reaction will occur both in the forward and reverse
...
If you know $\LaTeX$, I'd highly recommend right-clicking on the above equation and selecting "Show Math as TeX Commands". Feel free to laugh at my clunky implementation of getting an equilibrium sign, and understand that this kind of bodging is why we've never been first in Google search results :cry:
...
-   The Gibbs free energy change for the reverse reaction will be the
    negative of this value, $+33.3 kJmol^{−1}$, so we would predict that
...
{{< figure src="https://cdn.statically.io/gh/psharma04/image-repo/main/uploads/maxwell-boltzmann-graphwpcMYG.svg" theme="light" >}}
...
> {{< figure src="https://cdn.statically.io/gh/psharma04/image-repo/main/uploads/catalyst-reaction.min.svg" theme="light" >}}
...
-   deduce the equilibrium expression (in terms of $K_{eq}$) for
    homogeneous reactions occurring in solution (ACSCH079, ACSCH096) 

explore the use of $K_{eq}$ for different types of chemical
...
-   If K is very small, then $x$ will be very small, and the calculation
    can be simplified by using the following approximation:

> $[Reactant]_{initial} − x \approx [Reactant]_{initial}$

-   However, the assumption must be checked to be valid. The
    calculated value of $x$ must be less than 5% of the reactant's
...
$\ce{(Fe(NO3)3)}$and potassium thiocyanate $\ce{(KSCN)}$ combine, in a
reversible exothermic process, to form the aqueous iron(III)
thiocyanate complex $(\ce{\ce{Fe(SCN)^2+}(aq)}).$
...
A shift to the RHS, increase concentration of $\ce{Fe(SCN)^2+}$ → More
intense
...
A shift to the LHS, decreases concentration of $\ce{Fe(SCN)^2+}$ → Less
intense
...
{{< figure src="https://cdn.statically.io/gh/psharma04/image-repo/main/uploads/Spectrophotometer.min.svg" theme="light" >}}
...
-   $I_{0}$: The intensity of light passing through the blank
    (reference) sample.
-    I: The intensity of light passing through
    the analyte sample.
...
-   ε: Extinction coefficient (also known as molar absorptivity), a
    constant that relates absorbance with concentration and path length,
    with the units $L/cm/mol$
...
-   However not all ionic compounds are soluble in water. For example,
    $\ce{AgCl}$ and $\ce{Ca3(PO4)2}$ are insoluble.
...
entropically favourable $(∆S \gt 0)$, but some dissolutions are
unfavourable.
...
> {{< figure src="https://cdn.statically.io/gh/psharma04/image-repo/main/uploads/solubility-curves.min.svg" theme="light" >}}
...
expressing \[$\ce{H+}$\] as a number generally between 0 (extremely acidic)
